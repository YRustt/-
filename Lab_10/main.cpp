...
void unique(std::string& in_filename, std::string& out_filename) {
...
uint32_t get_root() {
    std::ifstream forward("forward.bin", std::ios::in | std::ios::binary);
    std::ifstream backward("backward.bin", std::ios::in | std::ios::binary);
...
void make_files(std::string& filename) {
...
int main() {
    merge_sort<Node>("input.bin", "tmp.bin");
    unique("tmp.bin", "forward.bin");

    reverse_pair();
    merge_sort<Node>("reverse_input.bin", "tmp.bin");
    unique("tmp.bin", "backward.bin");
